...
**The whole-body grid is the fallback spelling, and it is not equivalent.** `grid => <T, N> [[T; N]; N]` resolves too, deriving one open synthetic and two closed entries — and **no instantiation entry at all**. Its own body is an `instance_template`, so closing it produces the constructor body those bindings described and names that entry for the *form*; the field points straight at the array and nothing anywhere records that `grid<pixel, 3>` was written. Both halves follow D6 — a closed synthetic's `source` names the constructor it builds, an instantiation entry's names the application — but the consequence is worth stating rather than discovering: two spellings of what an author would call one type differ in whether the application survives into §8 output. That is why the record form is primary here.
