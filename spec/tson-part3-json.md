...
3. **Record extension, and `@discriminator` on a field** ([TSON-SCHEMA] §5.2, §6, §9). A record states how it may be realised — OPEN, ABSTRACT, SEALED or FINAL — and a field of an abstract record may be marked a discriminator. A **sealed family** is the abstract case carrying one or more such fields: the base declares them unpinned, every subtype pins them `REQUIRED_FIXED`, the pins are pairwise distinct (as tuples, where there is more than one), and the value→variant mapping is reconstructed from those pins and never declared. Consumed by: §6.1.5, which reads the members to recover the subtype, and §8.2, whose predicate is one route shorter for it. **The mapping is a mapping of a type-system capability and not a JSON mechanism**: the base's instantiability and the selector's identity are facts of the model, so both encodings read them, and this document adds only the member spelling. *This is the one item here whose owning document does not yet carry it as drafted — the retargeting from choice declarations to record families is an open proposal against [TSON-SCHEMA], and §6's `@discriminator` is stated there over choices. This document is written against the family form, which is the form it can implement.*
