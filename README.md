...
```tson
!!id:"https://example.com/2026/36/getting-started/person.tn?sha256=043301918795dcc349d156ae73a474fd047159b76d079859ee45f42446783c45"
!!meta:"https://tson.io/2026/36/m/meta.tn?sha256=711876037cfd3ebc8136d13218c28b40580ab8528423f9b4f2abe82380f737bb"
!!import:"https://tson.io/2026/36/m/core.tn?sha256=842da8ac938bf4d3071596bfaacfd517cb64c5735feb6005ca384b9605de957c"
...
### 6. Read the header first, then the body — `tson.begin`
...
and content sniffers can classify a document from its opening bytes" — `begin` is that door. It reads
...
if (peek.isSchemaDocument()) { … }                            // it carries !!meta
String schemaUri = peek.header().schema().orElse(DEFAULT);    // route on what it names
```
...
**The peek keeps the rest of the document**, so nothing is rewound and nothing is re-read — which is what
makes this work on a source that can only be read once, an HTTP request body or a socket. Hand the peek to
whichever reader the header selected:

```java
TsonDocumentPeek peek = v1.begin(request.getInputStream());
Tson version = isV2(peek.header()) ? v2 : v1;      // a schema version is a bind context, so a reader
Order order = version.objectReader().read(peek, Order.class);
```

That is the shape a receiver serving two API versions wants: one schema version per `DataBindContext`, and
the choice made after the header and before the body. The continuing reader must share the policy the header
was read under — §8.2's token policy and §9.1's limits already governed those tokens — and is refused if it
does not. Only the header is buffered, never the document; a 500 KB body still streams.
...
[design/schema-resolution.md](design/schema-resolution.md) for the full walkthrough, including how
