 ...
 * dependency on {@code compiler.TsonParserFactoryRegistry} (which is going away) or anything else in
 * {@code compiler} -- deliberate, while this package's own shape is still settling.
 *
 ...
 * with the working entries above) still have no compiled compiler at all -- {@code extern}, {@code
...
public final class ValueReaderFactoryRegistry implements ValueReaderFactoryResolver {
    ...
    private static Map<String, ValueReaderFactory> baseFactories(ValueReaderFactory record, ValueReaderFactory array,
        ...
        // ---- meta-kernel.tn1/meta.tn1 with no compiled compiler at all. Registered to ErrorReader so a
    ...
    private static ValueReaderFactory notImplemented(String constructorName) {
        return (name, typeDefinition, resolver) -> new ErrorReader(name, new UnsupportedOperationException(
                "'" + name + "' uses the '" + constructorName + "' constructor, which has no compiled compiler "
                        + "implemented yet"));
    }
}
