 ...
 * `Optional<List<T>>` (or any `Optional` wrapping a parameterized collection type) is not supported: `DefaultRecordBinder.resolveOptionalField` casts `Optional`'s actual type argument straight to `Class<?>`, which fails for a `ParameterizedType` like `List<String>` (works fine for `Optional<String>`/`Optional<SomeRecord>`, where the type argument really is a plain `Class`). Found in `ltr8-io-tson-java`'s `tson-schema` module, where a record field wanting "an empty list is absent, omit it" treatment for a `List`-typed field couldn't get it the way `Optional<SomeRecord>` fields already do.
 * Mutually-recursive record types cause infinite recursion (`StackOverflowError`), not a clean error: `DataBindContext.getDescriptor` only caches a class's descriptor *after* `resolve()` returns, and `DefaultRecordBinder` eagerly resolves every field's descriptor while building a record's own -- so two records that reference each other's type (directly, or through a field's generic argument) recurse forever the first time either is resolved. `DefaultUnionBinder` already works around exactly this for unions (its own code comment: "members are not resolved at this point as we can end up in an infinite loop... by using the actual member classes the resolution loop is broken") -- plain records have no equivalent guard. Found in `ltr8-io-tson-java`'s `tson-schema` module (`TypeRef`/`TypeArgument`, mutually recursive by design, e.g. nested generic applications like `array_ranged<array_ranged<T, N, N>, N, N>`); worked around there by keeping `TypeArgument` a sealed interface (a union breaks the cycle) instead of the more literal plain-record translation of its source shape, at the cost of a spurious type-ref tag on write. A real fix here (cycle detection, or a placeholder/lazy descriptor registered before recursing into fields) would let that workaround be removed.
 * A sealed interface's union members are collected one level deep only, not recursively: `DefaultUnionBinder.resolveUnion` calls `targetClass.getPermittedSubclasses()` exactly once and uses that array verbatim as the union's member list -- if a permitted subclass is itself a sealed interface (rather than a concrete, directly-instantiable class), it's kept as a "member" as-is instead of being walked into to find the concrete leaves underneath. Both directions break silently-ish: writing throws `"value of type X is not a member of union"` for every real instance (its runtime class is never one of the intermediate sealed interfaces), and reading can never match a type-ref name to anything, since `@Typename`/simple-name matching only scans that same flat, one-level list. Found in `ltr8-io-tson-java`'s `tson-schema` module, designing a multi-level kind hierarchy (`Top` permits `Atom`/`Product`/`Sum`/`Reference`, `Atom`/`Product`/`Sum` each further permit their own concrete leaf records) meant to mirror the TSON kernel's own `atom`/`product`/`sum` composition chain -- using `Top` as a bound field's type instead of a flat, single-level union was the goal, but isn't usable today for exactly this reason; worked around by keeping the flat union (`TypeBody`) as the actually-bound type and layering `Top`/`Atom`/`Product`/`Sum` on the side as an additional, non-bound hierarchy purely for `instanceof`-based kind checks in application code. A real fix here (recursing into any permitted subclass that `isSealed()`, flattening down to concrete members before building the union) would let a bound field use a multi-level sealed hierarchy directly.
 * `java.util.regex.Pattern` has no built-in data conversion (i.e. no auto-detection the way `Class#isEnum()` gets `EnumStringBridge` in `DefaultClassBinder`): `DefaultClassBinder.resolve` still throws `"Unable to find a valid data conversion for class: class java.util.regex.Pattern"` for a bare, unregistered `Pattern`/`Optional<Pattern>` field. First surfaced in `ltr8-io-tson-java`'s `tson-schema` module (`TextType.pattern`/`UriType.pattern`, both originally `Optional<Pattern>`) once those became real bind targets (`MetaKernelParser`'s meta-kernel bootstrap resolving `text`/`uri`/`regex` to their `UNCONSTRAINED` constants) -- `TsonMapper.toTson` threw for all three entries even with the field absent, since record binding resolves every field's *descriptor* up front (the same eagerness noted above) regardless of whether a `Pattern` value is ever actually present. `PatternStringBridge` (`io.ltr8.bind.bridge`, mirroring `EnumStringBridge`'s shape -- `toData`/`toObject` via `Pattern.pattern()`/`Pattern.compile(...)`) now exists and is unit-tested (`PatternStringBridgeTest`), usable via `context.registerAtom(Pattern.class, new PatternStringBridge())` the same explicit-opt-in way `UUIDBridge` works for `UUID`. **`ltr8-io-tson-java`'s own case ended up not needing it**, though: `regex_type` composes with `text_type` (Part 2 §5.7) -- a `regex` value IS-A piece of text -- so `TextType.pattern`/`UriType.pattern` moved to `Optional<String>` (the pattern's own source text) instead, which binds natively with no bridge at all. `PatternStringBridge` is provided as a general library capability regardless, for a caller who does want to keep `Pattern` as their own Java-side field type.
