...
import io.ltr8.tson.parser.resolver.vocab.FloatType;
 ...
 * vocabulary type ({@link FloatType}) purely to reuse its {@code .nan}/{@code +.inf}/{@code -.inf}
 ...
 * for it here, unlike every other use of {@code FloatType}.
 */
final class AtomWriter {
    ...
    static void writeDefaultAtom(Object value, TsonWriter writer) throws DataBindException {
        switch (value) {
            ...
            case Double d -> writer.unquotedToken(FloatType.FLOAT64.write(d));
            case Float f -> writer.unquotedToken(FloatType.FLOAT32.write(f));
