 ...
 * of the type system and not of the encoding that carried them. That is why it lives here rather than beside
 * either engine: this module already holds {@code HostAtoms}, the index from these same classes back to the
 * family that produces each, and a consumer whose class has a {@code UUID} component must not have to
 * discover that one front door binds it and the other does not.
 *
 * <p>Every type here is registered as an <b>atom</b> rather than left to {@code tson-bind}'s structural
 * auto-detection, for two different reasons. Most are opaque scalars the engine has no way to take apart.
 * Two would otherwise be taken apart wrongly:
 *
 * <ul>
 *   <li>{@link CidrNetwork} is a Java record, so auto-detection would make a
 *       {@code { prefix: ... prefixLength: ... }} of it and refuse the scalar {@code cidr4}/{@code cidr6}
 *       actually carry.</li>
 *   <li>{@link Token} <em>is</em> one token on the wire -- the text plus the form that produced it --
 *       where binding it structurally writes {@code { text: ... form: ... }}, a record where
 *       [TSON-SCHEMA] §8's resolved form has a scalar. Reading is unaffected, the slot's own reader
 *       deciding; this is what stops a writer inventing a shape.</li>
 * </ul>
 *
 * <p>{@code duration_type} and {@code period_type} carry their bounds as values rather than tokens -- the
 * two families compare the value and not the spelling -- so both host types are components of a
 * {@code schema.meta} record and need registering, as the other temporal host types do.
 *
 ...
 *
 * <p><b>What is not here</b> is the {@code SourcePosition} bridge, which names {@code tson-compiler}'s own
 * {@code Position} and so cannot travel; that engine registers it on top of this list, in
 * {@code config.ResolverBindContext}. The split is by what needs it: this is what a <em>consumer's</em>
 * classes bind, and the addition is what binding this library's own schema model additionally needs.
 */
