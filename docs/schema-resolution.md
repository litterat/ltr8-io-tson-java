...
by entry identity, exactly as at link time (`spec/tson-rev33-changelog.md` #55, and `docs/linking-and-compilation.md`
...
- **An annotation on a declaration resolves one hop and only one hop** (§3.3.3): against the governing
  ...
  **data documents** and not within the schema document itself, so the message names the remedy (move the
  ...
  and the metadata was not there (`spec/tson-rev33-changelog.md` #56, `SchemaAnnotationScopeTest`). A value that *does*
...
- **Subtraction runs last and breaks IS-A on purpose** (§5.9). Supertypes merge, the body adds and tightens,
  ...
  **emptied** (the contract — so §7.2's subsumption check won't let a subtracted type stand where its source
  ...
  against §4.3's "composition grants IS-A per parent"; `spec/tson-rev33-changelog.md` #37 has the per-ancestor
  ...
  members left is dropped — the one arity §5.11 doesn't legislate (`spec/tson-rev33-changelog.md` #36).
- **Chained atom refinement merges with the source, it does not replace it** (`spec/tson-rev33-changelog.md` #17):
...
- **A refinement must narrow, and this is enforced** (§5.7). After binding, `checkNarrows` asks the
  ...
  rejecting one would reject a documented construct (`spec/tson-rev33-changelog.md` #27).
...
- **Two exception types, and which one is deliberate.** `UnsupportedOperationException` means *this library
  ...
  `refined-def` takes a name — `spec/tson-rev33-changelog.md` #38), a name in a `!` position resolving in neither
...
- **Substitution is where a routed `=` becomes fixed.** §5.7 puts a parametric `= P` in `REQUIRED` at the
  ...
  refinement-from-an-application-head case downstream and leaves this one unstated (`spec/tson-rev33-changelog.md` #58).
...
- **A reference template composes rather than minting an entry.** §5.10's *partial application* —
  ...
  - **A self-applying alias is the author's error, not a knot.** The knot-tying memo answers a recursive
    ...
    reference-template applications in flight and reports the cycle instead (`spec/tson-rev33-changelog.md` #45 names
