...
/**
 * meta-kernel's {@code regex_type} constructor ({@code ~text_type & atom_specification & { spec: =
 * "https://www.rfc-editor.org/rfc/rfc9485" }}) -- reuses {@link TextType}'s length/pattern
 * constraint checks (applied to the regex's own source text, not what it matches) via composition
 * rather than duplicating them, and additionally requires that text to actually compile as a regular
 * expression. Like {@link TextType}, not part of Part 1's published built-in vocabulary (§5) and
 * never registered in {@link BuiltinTypeVocabulary} -- this is groundwork for Part 2, which doesn't
 * yet have anything that consumes a {@code regex} value as a constraint.
 *
 ...
 */
public record RegexType(TextType constraints) implements AtomType<Pattern> {

    /** {@code regex => !regex_type {}} -- the unconstrained regex type. */
    public static final RegexType UNCONSTRAINED = new RegexType(TextType.UNCONSTRAINED);

    @Override
    public Pattern read(TokenValue token) {
        String text = constraints.read(token);
        try {
