...
/**
 * meta-kernel's {@code time_type} constructor (§5.4's {@code time} atom, RFC 3339 {@code
 * full-time}). Same shape-then-delegate pattern as {@link DateType} -- {@link OffsetTime#parse}
 * itself already gets RFC 3339's case-insensitive {@code T}/{@code Z} allowance right natively (no
 * extra work needed there, confirmed empirically) and correctly requires the offset ({@code
 * "10:15:30"} with no zone is rejected), but needs the same year-shape guard {@link DateType} does
 * where a date is involved -- moot for a bare time, so the regex here only needs to anchor the
 * overall shape, not work around a JDK leniency the way {@code DateType}'s does.
 *
