 ...
 * Indirect} exists purely for the edges {@link TsonSchemaParser}'s own build-stack finds still
 ...
 * TsonSchemaParser#get} call that triggered this whole construction chain has returned, instead of
 * the object it can't hold yet.
 *
 * <p>Both variants are themselves a {@link TsonTypeParser}, so a composite parser holding a {@code
 ...
 */
public sealed interface ParserHandle<T> extends TsonTypeParser<T> {

    /** The non-cyclic case -- {@code parser} was already finished when this handle was created. */
    record Direct<T>(TsonTypeParser<T> parser) implements ParserHandle<T> {
     ...
     * the compiler's own {@code Map<String, TsonTypeParser<?>>}, captured by reference, not copied
     * -- by the time {@link #read} is ever actually called, the specific {@link
     * TsonSchemaParser#get} call whose own recursive construction created this handle in the first
     ...
     * This still holds under {@link TsonSchemaParser}'s own lazy compilation: it was never "the
     ...
     */
    record Indirect<T>(String typeName, Map<String, TsonTypeParser<?>> registry) implements ParserHandle<T> {

        @Override
        @SuppressWarnings("unchecked")
        public T read(DataValue value) {
            TsonTypeParser<?> resolved = registry.get(typeName);
            if (resolved == null) {
