 ...
 * {@code io.ltr8.tson.parser.resolver.schema.compiled.RecordShapeFactory}, in a different package
 ...
 * `resolver`'s own "shared behind more than one downstream consumer" spot) rather than in
 * `resolver.schema.compiled` itself, since `mapper` needs it too and `mapper` doesn't depend on
 * `compiled`.
 */
