 ...
 * class (see {@code SchemaResolver}'s own Javadoc for that use).
 *
 ...
 * tson-parser} (this module) so schema resolution (constructor application, atom refinement, §5.5)
 * can use this binding directly without a module cycle ({@code tson-mapper} depended on {@code
 * tson-parser}, so the reverse was impossible) -- {@code tson-bind}, what this is built on, has no
 * dependency on {@code tson-parser}/{@code tson-schema} at all, so depending on it directly here is
 * clean.
 *
