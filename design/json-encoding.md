...
| `io.ltr8.tson.json.reader` | internal — the two schemaless engines (`SchemalessTreeReader`, `DataClassObjectReader`); the schema-directed readers, named mode first (`TreeAtomReader`, `TreeRecordReader`, `TreeArrayReader`, `TreeTupleReader`, `TreeMapReader` over `TreeMapObjectReader`/`TreeMapPairsReader`); the dispatchers every mode shares (`DispatchFactories` over `DispatchTagReader`/`DispatchMemberReader`, `DispatchChoiceReader`, with `Route` and `ScannedReader`) with `AtomReader`, `AtomForm`, `VoidReader`, `ValuePositionReader`, `DeferredTypeReader`, `OpenTemplateReader`, `ErrorReader`; the factory registries (`ValueReaderFactory`, `ValueReaderFactoryRegistry`, `ValueReaderFactoryResolver`, `ValueReaderContext`, `TypeReaderResolver`, `CompiledReaders`); and what they share — `ReservedMembers`, `Tags`, `NameHygiene`, `ReferenceChain`, `DiscriminationClass`, `ValueIdentity`, `FieldValue`, `Nodes`, `EventSkip` |
