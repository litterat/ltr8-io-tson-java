 ...
 * five components flattened directly onto it) -- a {@code LinkedTsonSchema} *is* a {@code
 ...
 * not necessarily linked"; a {@code LinkedTsonSchema} means "linked." Keeping them nominally
 * distinct means {@link SchemaRegistry#register} can declare it only accepts a {@code
 * LinkedTsonSchema} and the compiler enforces "you must link before you register" -- no flag to
 * check, no way to forget.
 *
 * <p><b>{@link SchemaRegistry} stores this type directly</b> -- every entry it ever holds arrived
 * via {@link #register}, so every entry is, by construction, linked; there's no other kind of
 * thing to store. {@link SchemaLoader#load} returns this type for the same reason.
 */
public record LinkedTsonSchema(TsonSchema schema) {

    public LinkedTsonSchema {
