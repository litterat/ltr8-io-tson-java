...
/**
 * meta.tn1's {@code unknown_type} constructor ({@code unknown_type => ~sum & {}}) -- an empty
 * SUM-kind marker whose instance, {@code unknown} (core.tn1), accepts any well-formed value of any
 ...
 * class so far does). This is what widened {@code TsonSchemaResolver}'s own {@code Instance} resolution
 * from binding against {@code Atom.class} specifically to {@code Top.class} -- previously a
 * deliberate scope limit ("every real Instance in core.tn1/meta.tn1 targets an atom-family
 * constructor," flagged as "a one-word change if it's ever needed") -- see {@code TsonSchemaResolver}'s
