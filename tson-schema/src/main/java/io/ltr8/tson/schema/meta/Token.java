 ...
 * which of the three token forms produced it, mirroring {@code tson-compiler}'s own {@code
 * TokenValue}/{@code TokenForm} shape exactly (same field name, same enum members) but declared
 * locally here rather than imported: {@code schema.meta} has no dependency on {@code tson-compiler}
 * -- {@code tson-compiler} depends on {@code tson-schema}, not the reverse (see this repo's own
 * CLAUDE.md) -- so it needs its own, structurally-identical stand-in rather than reusing that type
 * directly. Callers on the {@code tson-compiler} side (e.g. its schema resolver) convert their own
