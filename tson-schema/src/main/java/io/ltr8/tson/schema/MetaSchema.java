 ...
 * own. Produced by {@code io.ltr8.tson.parser.resolver.schema.MetaKernelParser} (in {@code
 * tson-parser}) -- meta-kernel's own {@code !!meta} names itself, so it can't come from ordinary
 * {@code SchemaResolver} resolution alone (§1.5's "one deliberate circularity in the series,
 * closed by pre-loading rather than by resolution").
 */
